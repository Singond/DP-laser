## -*- texinfo -*-
## @deftypefn  {} {@var{in} =} normalize_intensity (@var{raw}, @var{accums})
## @deftypefnx {} {@var{s} =} normalize_intensity (@var{s})
##
...
## @end deftypefn
function r = normalize_intensity(varargin)
	if (nargin == 0)
		print_usage;
	end
	...
	if (isstruct(varargin{1}))
		r = varargin{1};
		r.in = normalize_intensity(r.img, r.acc);
		return;
	end
...
%!assert(normalize_intensity(40*img, 40), img);

%!test
%! img_nd = repmat(img, 1, 1, 3);
%! in = normalize_intensity(40 * img_nd, 40);
%! assert(in, img_nd);
