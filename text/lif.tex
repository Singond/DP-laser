...
\section{Úvod}
\label{sec:lif-intro}
Druhým provedeným experimentem bylo určení absolutní koncentrace
...
Atomizace je rozklad molekul sloučeniny na jednotlivé atomy v~plynné fázi.
V~analytické chemii je to běžně používaný proces užitečný při určení
koncentrace určitých atomů ve vzorku:
Neznámý vzorek přivedený v~plynné podobě je atomizován a vzniklé
volné atomy jsou podrobeny zkoumání,
při němž mnohdy hrají nemalou roli optické spektrální metody.
Rozklad na atomy výrazně usnadňuje detekci částic,
neboť atomová spektra jsou jednodušší, méně závislá na teplotě
a~leží blízko oblasti viditelného světla.
To je podstatný rozdíl oproti spektrům molekulovým,
která na teplotě závisejí daleko silněji.
...
Pokus byl proveden ve spolupráci s~Ústavem analytické chemie
Akademie věd České republiky, která se atomizací zabývá a~poskytla
chemickou aparaturu.
...
Směs byla čerpána do generátoru hydridů, kde se setkávala
s~\num{0.5}\si{\percent} roztokem borohydridu sodného (\ce{NaBH4})
v~\num{0.4}\si{\percent} hydroxidu draselném (\ce{KOH}).
Hydroxid původně sloužil ke stabilizaci jinak velmi reaktivního borohydridu,
ale v~přítomnosti kyseliny byl neutralizován a~borohydrid začal působit
na selen jako redukční činidlo.
Selen se tak redukoval na selan (tj.~hydridu selenu, \ce{H2Se}).
Přebývající vodík pocházející z~borohydridu se vyloučil
v~plynné podobě (\ce{H2}).
...
Vodík vznikající reakcí borohydridu s~kyselinou by nestačil pro udržení
plamene (jeho průtok byl odhadován na \SI{11}{\sccm}),
proto byl bočním kanálem atomizátoru do směsi přiváděn další vodík
o~volitelném průtoku, který obvykle činil \SI{300}{\sccm}.
Nad ústím atomizátoru se vodík mísil se vzdušným kyslíkem
a~hořel plamenem, v~němž vznikaly volné atomy vodíku,
způsobující ve dvou krocích
atomizaci selenu rozpadem selanové molekuly \ce{H2Se}:
\begin{align*}
	\ce{
		H + H2Se &-> H2 + HSe \\
		H + HSe &-> H2 + Se
...
Jako dělič sloužila křemenná destička,
...
za dělič (kde je svazek silnější) a~energie svazku za~atomizátorem není měřena.
...
Použitý model kamery byl \instrname{PI-MAX: 1024} od Princeton Instruments.
...
\begin{figure}[p]
	\centering
	\input{../lif/results/rayleigh-example-small}
...
\begin{figure}[p]
	...
	\caption{Detail maxima integrálního excitačního profilu
		...
		Parametry proložených profilů
		jsou v~tabulce č.~\ref{tab:lif-excitprof-fit}.}
...
Absorpční čára se potom blíží $\delta$-funkci
...
Obdobný postup, pak vede na vodorovně rozlišené
...
\begin{figure}[htp]
	...
	\caption{Časový vývoj fluorescenčního signálu.
		...
		který je mírnější ve středu plamene
		(kolem $\xpos = \SI{100}{\pixel}$).}
...
Prvními jsou vzdušný kyslík a~dusík, jež sem pronikají z~okolní atmosféry,
dalším je vodní pára, která vzniká spalováním vodíku.
Její koncentrace může být v~okrajových částech vyšší v~důsledku
vyšší koncentrace kyslíku a~tudíž rychlejšího spalování.
Kyslík, dusík a~zejména vodní pára zhášejí excitované stavy mnohem rychleji
...
Jeho hodnota pro svisle polarizované světlo, jaké měl laserový svazek,
byla určena podle \cite{rayleigh} jako:
\begin{equation}
...
Nízká intenzita vlastního záření plamene je při měření výhodou,
neboť nevede k~nadměrně velkému signálu pozadí.
...
Je dobře patrno, že vyšší průtok plynu obecně vede k~delšímu plameni.
Dvě sady pro průtok argonu \SI{175}{\sccm} mají podobné maximum koncentrace
(zhruba \SI{5e18}{\per\metre\cubed} ve výšce \SI{2}{\milli\metre} nad ústím),
ale v~sadě s~vyšším průtokem vodíku koncentrace klesá směrem vzhůru pomaleji
a~plamen je delší.

Důležitým poznatkem je, že atomární selen se nachází i~uvnitř plamene,
nikoli jen po jeho okrajích, kde se předpokládá nejsilnější atomizace.
Jeho koncentrace je zde dokonce vyšší, přestože je středová oblast
relativně studená.
Možné vysvětlení je, že ve středu dochází k~atomizaci vlivem vodíkových
atomů, které sem difundují z~okrajové oblasti.
Druhý faktor je nižší teplota uprostřed plamene, která obecně
vede k~vyšší hustotě plynu.
Vně plamene podle předpokladu žádný selen detekován není,
neboť je zde rychle oxidován vzdušným kyslíkem.
