...
\begin{figure}[htb]
	\centering
	\includegraphics[width=\textwidth]{lif-setup-se}
...
\begin{figure}
	\centering
	\input{../lif/results/saturation-full-params}
	\caption{Parametry závislosti LIF na energii pulzu
		určené z~první sady dat
		(od \SI{2.0}{\micro\joule} do \SI{4.0}{\micro\joule}).
		Pro potlačení šumu byly snímky LIF před zpracováním vyhlazeny
		průměrováním ve čtverci o~rozměrech \num{5}\times\SI{5}{\pixel}.
		Temnější oblast uprostřed výšky snímku naznačuje nerovnoměrné
		rozdělení energie laserového svazku.
		Přesto je patrno, že saturační parametr $\lifsat$ je
		v~oblasti plamene ve vodorovném směru přibližně homogenní.}
	\label{fig:saturation-full-params}
\end{figure}
