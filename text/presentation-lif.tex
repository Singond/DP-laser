...
\begin{frame}
	\frametitle{Realizace LIF}
	\begin{columns}[c]
		\begin{column}{0.48\textwidth}
			\includegraphics[width=\textwidth]
				{lif.png}
			Zdroj: J.~Voráč, P.~Dvořák, M.~Mrkvičková
		\end{column}
		\begin{column}{0.48\textwidth}
			\begin{enumerate}
				\item laditelný pulzní laser
				\item hranol
				\item válcová rozptylka
				\item spojka
				\item zkoumané plazma
				\item měřič energie laseru
				\item synchronizovaná ICCD kamera
				\item laserový svazek
				\item Fresnelovy hranoly
			\end{enumerate}
		\end{column}
	\end{columns}
\end{frame}
...
\begin{frame}
	...
	\begin{columns}[c]
		\begin{column}{0.48\textwidth}
			\begin{itemize}
				\item selen ve~vodíkovém plameni
				\item roztok 10 ppb Se v~\SI{1}{\mol\per\litre} \ce{HCl}
				\item borohydrid \SI{0.5}{\percent} \ce{BH4}
					v~$0,4\%$ \ce{KOH}
				\item argon (\SI{700}{\sccm})
			\end{itemize}
		\end{column}
		\begin{column}{0.48\textwidth}
			\begin{figure}
